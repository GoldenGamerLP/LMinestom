...
public class LMinestomStopCommand extends Command {
    ...
    public LMinestomStopCommand(@NotNull String name, @Nullable String... aliases) {
        ...
        setDefaultExecutor((sender, context) -> MinecraftServer.getSchedulerManager().buildTask(() -> MinecraftServer.getConnectionManager().getOnlinePlayers().stream().filter(player -> player.hasPermission("lminestom.stop")).forEach(player -> player.sendMessage(Component.text(sender.asPlayer().getUsername() + "has shut the Server down!")))).delay(Duration.ofSeconds(5)));
